...
**Status:** In progress — on-page SEO is now complete across all five pages (landed 2026-08-19): generated `sitemap.xml`/`rss.xml`/`robots.txt` (Pillars 1 & 7), unique `<meta name="description">` and a standardized `<title>` convention (Pillar 2), `<link rel="canonical">` on every page (Pillar 1), and per-page Open Graph + Twitter Card tags with per-landmark 1200×630 images (Pillar 4). Performance and accessibility (Pillars 5-6) are audited: accessibility passes (pa11y-ci/axe 5/5 URLs, 0 errors) and Lighthouse SEO/best-practices/accessibility clear the 0.9 gate; mobile **performance** on the image-heavy landmark pages still sits below 0.9 (raw hero-JPEG byte weight), pending responsive-image/recompression work now owned by [`asset-loading-optimization.md`](./asset-loading-optimization.md). Only multilingual/`hreflang` (see Still open) remains.
...
5. **Performance & Core Web Vitals**
   - ✅ **Audited [2026-08-19].** At audit time: landmark heroes use `fetchpriority="high"` + `decoding="async"` (LCP); the `ottoman/index.html` hub-grid thumbnails carry `width`/`height` + `loading="lazy"` + `decoding="async"`; Lighthouse SEO/best-practices/accessibility clear the 0.9 gate. *Note (superseded):* the home page (`src/index.html`) was later reworked into the Parts grid; its images initially lost the `loading`/`decoding` hints (CLS remained reserved via CSS `aspect-ratio`), since re-added — along with the broader recompression/modern-format/responsive work — under item 3 of [`asset-loading-optimization.md`](./asset-loading-optimization.md), not here.
   - **Remaining:** mobile performance on the image-heavy landmark pages is still below 0.9 (bridge ~0.73, fountain ~0.76, monastery ~0.84) due to raw hero-JPEG byte weight (e.g. `prizren_bridge.jpg` ~948 KB). Fixing it needs recompression, modern formats (AVIF/WebP), and responsive images (`<picture>`/`srcset`) — now owned by [`asset-loading-optimization.md`](./asset-loading-optimization.md) (which also covers the previously-untracked font/LCP `preload` hints). The Lighthouse gate is warn-only, so this doesn't block.
