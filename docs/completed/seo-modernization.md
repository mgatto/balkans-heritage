...
**Status:** In progress — on-page SEO is now complete across all five pages (landed 2026-08-19): generated `sitemap.xml`/`rss.xml`/`robots.txt` (Pillars 1 & 7), unique `<meta name="description">` and a standardized `<title>` convention (Pillar 2), `<link rel="canonical">` on every page (Pillar 1), and per-page Open Graph + Twitter Card tags with per-landmark 1200×630 images (Pillar 4). Performance and accessibility (Pillars 5-6) are audited: accessibility passes (pa11y-ci/axe 5/5 URLs, 0 errors) and Lighthouse SEO/best-practices/accessibility clear the 0.9 gate; mobile **performance** on the image-heavy landmark pages still sits below 0.9 (raw hero-JPEG byte weight), pending responsive-image/recompression work deferred to the `<picture>`/`srcset` item in `html-webcomponents-mastery-checklist.md`. Only multilingual/`hreflang` (see Still open) remains.
...
- **Canonical URLs: done.** ✅ [2026-08-19] `<link rel="canonical">` on every page, using the apex origin (bare `https://balkanheritage.info/` for home, `/<page>.html` for landmarks).
...
5. **Performance & Core Web Vitals**
   - ✅ **Audited [2026-08-19].** All `<img>` now carry intrinsic `width`/`height` (CLS); landmark heroes use `fetchpriority="high"` + `decoding="async"` (LCP); homepage grid thumbnails use `loading="lazy"` + `decoding="async"`. Lighthouse SEO/best-practices/accessibility clear the 0.9 gate.
   - **Remaining:** mobile performance on the image-heavy landmark pages is still below 0.9 (bridge ~0.73, fountain ~0.76, monastery ~0.84) due to raw hero-JPEG byte weight (e.g. `prizren_bridge.jpg` ~948 KB). Fixing it needs responsive images (`<picture>`/`srcset`) and/or recompression — deferred to that item in [`html-webcomponents-mastery-checklist.md`](html-webcomponents-mastery-checklist.md). The Lighthouse gate is warn-only, so this doesn't block.
