...
- `npm run convert:audio` — transcode the glossary pronunciation mp3s under `src/public/assets/audio/{al,bs}/` to WebM/Opus and M4A/AAC (same basename, alongside the mp3, which stays as the legacy fallback). Source clips shorter than 1.2 s are first padded in place with trailing silence (idempotent): native `<audio>` controls render whole seconds, so a sub-second duration would display as 0:00. Requires ffmpeg + ffprobe on PATH (`brew install ffmpeg`); incremental, `--force` to rebuild, `--dry-run` to print commands. Re-run whenever a clip is re-recorded or added.
...
- **`src/public/`** is copied verbatim to the site root (manifest, `humans.txt`, `_redirects`, `_headers`). Audio files live under `src/public/assets/audio/{al,bs}/` and are copied to `dist/assets/audio/` — they won't exist in `src/assets/` during development. Each pronunciation clip ships as three files with the same basename (`.webm` + `.m4a` + `.mp3`); the glossary `<audio>` elements list them as `<source>`s in that order, with `property="contentUrl"` on the mp3 `<source>` so the RDFa triple is preserved. Regenerate with `npm run convert:audio`.
- **Audio (`/assets/audio/*`) is served through a Worker, not plain static assets.** Cloudflare's static-assets server ignores HTTP `Range` headers entirely (never returns `206 Partial Content`, verified in production and in `wrangler dev`), and Safari's `<audio>` controls require a 206 to display clip duration — so `wrangler.jsonc` sets `main: src/worker.js` and `assets.run_worker_first: ["/assets/audio/*"]`, and the Worker slices byte ranges itself. The Worker also normalizes `Content-Type` for the three audio extensions, because Cloudflare's extension-based mapping serves the audio-only `.webm` clips as `video/webm`. Consequences: custom `_headers` rules don't apply to those responses (the audio `Cache-Control`/`Accept-Ranges` live in `src/worker.js`, not `_headers` — don't re-add an `/assets/*.mp3` rule there); audio requests count as Worker invocations for billing (trivial at this traffic, but note the free-tier 429 caveat in Cloudflare's `run_worker_first` docs); and `wrangler deploy` now uploads a Worker script alongside the assets.
